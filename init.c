...
int main() {
    ...
    // printf("tmp: %hd\n", tmp);
    // 将超级块的信息写入磁盘文件
    if (1 != fwrite(&m_sb, sizeof(struct super_block), 1, disk_file)) {
    ...
    fread(&inode_first_byte, 1, 1, disk_file);
    ...
    printf("文件指针目前的位置：%ld\n", ftell(disk_file));
    // 重新写回文件
    if (1 != fwrite(&inode_first_byte, 1, 1, disk_file)) {
        perror("inode位图区更新失败\n");
        return 1;
    }
    ...
    */
    ...
    fwrite(&root_dir, sizeof(struct inode), 1, disk_file);
    fseek(disk_file, BLOCK_SIZE * m_sb.first_inode, SEEK_SET);
    printf("根目录初始化成功！\n");
